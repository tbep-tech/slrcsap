...
#' @param ybrk numeric, number of y-axis breaks. Default is `7`. Does not apply if `plotly = TRUE`.
...
plot_scenario <- function(dat, cols = c('deepskyblue', 'orange', 'red'), units = 'ft', linewidth = 1, 
                          caption = TRUE, xrng = c(2020, 2100), xbrk = 10, yrng = NULL, ybrk = 7, plotly = FALSE){
  ...
  if(plotly){
    ...
    p <- plotly::plot_ly(
        ...
        hoverinfo = 'text',
        text = ~paste0('Year: ', year, '<br>RSLC: ', round(yvl, 2), ' ', units)
      ) |>
      plotly::layout(
