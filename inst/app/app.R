...
gauge_options <- list(
  ...
  "Naples, FL" = 8725110,
  "Tampa Bay, FL" = 8726607,
  "Clearwater Beach, FL" = 8726724,
  "Anna Maria Island, FL" = 8726384
)
  ...
  nav_panel(
    ...
    layout_sidebar(
      ...
      card(
        ...
        card_body(
          plotOutput("sealevel_plot", height = "450px")
        )
      ),
  ...
  nav_panel(
    ...
    layout_sidebar(
      ...
      card(
        ...
        card_body(
          plotOutput("scenario_plot", height = "450px")
        )
      ),
    ...
    layout_columns(
      ...
      card(
        ...
        card_body(
          ...
          h6("Gauge Stations:"),
          p("The app includes 8 pre-configured Florida stations, or you can enter any NOAA gauge ID.")
        )
      )
    )
  )
...
server <- function(input, output, session) {
  ...
  output$sealevel_plot <- renderPlot({
    if (is.null(values$sealevel_data)) {
      ...
      tryCatch({
        plot_sealevel(values$sealevel_data, units = input$units) +
          ggtitle(paste("Historical Sea Level Data - Gauge", values$current_gauge)) +
          theme_minimal() +
  ...
  output$scenario_plot <- renderPlot({
    if (is.null(values$scenario_data)) {
      ...
      tryCatch({
        plot_scenario(values$scenario_data, units = input$units2) +
          ggtitle(paste("Sea Level Rise Projections - Station ID", unique(values$scenario_data$id))) +
          theme_minimal() +
          theme(plot.title = element_text(size = 14, hjust = 0.5),
